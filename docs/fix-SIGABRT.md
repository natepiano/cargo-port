...
**Constraints from prior phases:** Phase 8 removed the final production abort and `scripts/check-no-test-abort.sh` now expects zero abort calls. Phase 7 removed test/helper aborts. Quiet startup, `TestApp` fixture-owned startup isolation, lint supervisor joins, keymap fixtures, the Phase 6 repetition script, and the final zero-abort inventory are in place. `make_app()` and `make_app_with_config()` still return plain `App`, but `into_quiet_app()` now hands the fixture temp directories to that `App` through the test-only `App::fixture_dirs` field — declared last on the struct so it drops after every field that reads those paths — instead of leaking them with `std::mem::forget`; a full `cargo nextest run` therefore leaves no `.tmp*` directories in `$TMPDIR`. Phase 9 is validation-only unless validation reveals dispatch-critical drift that must be recorded in this plan.
