...
**Pending decision:** Stale monitor data silently becomes actionable again on the next scope replacement

Actual problem:
`MonitorSnapshot::superseded_by_scope` accepts `Stale(monitor_data)` as retention input and republishes it as `PendingWithRetained` (`src/build_monitor/snapshot.rs:306-318`), and `MonitorDataActionability` reports `PendingWithRetained` as `Actionable` (`:265-275`). Data that a failed refresh cycle deliberately aged to non-actionable is therefore restored to full kill authority — and loses its staleness marker — by a single arrow-key press within the same workspace. Phase 8 made scope replacement the common case, and Phases 11 through 13 build termination authority directly on `actionability()`.

What exists now:
- A failed classification cycle ages `Fresh` → `Stale`, which renders with a visible staleness marker and is non-actionable.
- The user arrows to another row in the same workspace. `replace_scope` calls `superseded_by_scope`, which takes the `Stale` variant's retained data and republishes it as `PendingWithRetained`.
- That variant is `Actionable`, so a scope-wide kill issued in the next 500 ms acts on process data the monitor already declared untrustworthy, with no marker telling the user it is stale.

What should change:
- Either retention drops `Stale` to plain `Pending` — the retained rows disappear on scope change, which costs display continuity but never grants authority over stale data — or retained-from-stale keeps its non-actionable status and its marker through the replacement, which preserves the display but needs a variant (or a field on `PendingWithRetained`) that carries staleness across scope changes.

Recommendation:
Take the second option and make it a Phase 9 edit to `src/build_monitor/snapshot.rs`, which this phase already touches for the un-gating and the per-activity extension. Dropping to `Pending` blanks the display on every arrow-key press after any failed cycle, which is the retention behavior Phase 8 exists to provide; keeping the staleness through replacement preserves that retention and still refuses authority. Concretely: `superseded_by_scope` maps `Stale(data)` to a retained-with-staleness state that `actionability()` reports `NotActionable` and the Phase 9 renderer draws with the stale marker, while `Fresh(data)` continues to map to `PendingWithRetained`.
...
- `src/build_monitor/snapshot.rs` — extend `MonitorData`/`MonitorSessionRow` with per-activity and unattributed data, and remove the `#[cfg(test)]` gates on the read API this phase is the first production reader of.
...
Actual problem:
No phase owns lifecycle state that survives a poll cycle. Phase 8's `record_classification` replaces `monitor_snapshot` wholesale every cycle, and `BuildMonitor` stores only `monitor_snapshot` plus `live_session_ids` (`src/build_monitor/mod.rs:64-68`). A `Terminating` transition (this phase), a gone-after-signal tombstone for the selected build (Phase 12), and a scope-wide tombstone (Phase 13) would each be erased by the next cycle and again by `replace_scope`.

What exists now:
- `record_classification` (`src/build_monitor/poll.rs:33-51`) builds a fresh `MonitorData` per cycle and drops the prior one; nothing carries forward except the retention path `superseded_by_scope` uses.
