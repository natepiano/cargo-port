...
### Phase 11 — Owned-run termination actor and platform capability foundation · status: todo
...
### Phase 12 — Termination authorization, bounded transaction, and lifecycle overlay · status: todo

#### Work Order

**Goal:** `BuildMonitor` can mint opaque termination authorization from fresh actionable sessions, execute one frozen identity-revalidated bounded transaction through Phase 11's terminator, and hold per-session lifecycle state that survives snapshot replacement and reaches the renderer.

**Spec:**

- Define authority-bearing `ActionableBuild::{Owned(OwnedBuildTerminationAuthority), External(ExternalBuildTerminationAuthority)}` variants. The owned variant retains `OwnedRunId` plus Phase 11's actor-issued run-bound authorization; the external variant bundles confirmed scope attribution, strong root identity, lifecycle eligibility, and Phase 11's identity-bound platform capability. An identifier by itself is never action authority.
...
- **Own a lifecycle overlay on `BuildMonitor`, keyed by `BuildSessionId` and stored outside `monitor_snapshot`.** `record_classification` (`src/build_monitor/poll.rs:42-56`) builds a fresh `MonitorData` every poll cycle and drops the prior one, so a `Terminating` transition written into the snapshot is erased within ~500 ms and again on `replace_scope`. The overlay survives both, with explicit eviction rules rather than implicit clearing alongside the snapshot. Phases 13 and 14 store their gone-after-signal tombstones in this overlay instead of each inventing storage; Phase 14's `src/build_monitor/aggregate.rs` arrives too late to serve as that home.
- **Fold `live_session_ids` into the overlay's key set, or delete it.** It is a `BTreeSet<BuildSessionId>` derived from the snapshot's rows and cleared alongside them (`src/build_monitor/poll.rs:50-56, :68, :90`), reachable only through a `#[cfg(test)]` accessor, with no production reader and no other phase claiming it — a second answer to "which sessions are live" that this phase either adopts as the one answer or removes.
- **Carry the overlay to the renderer in this phase, not in a later one.** `OutputPresentation::derive` takes only `&MonitorSnapshot` plus owned-run state (`src/tui/app/mod.rs:1207-1213, 1221-1229`), and Phase 9's single-presentation constraint forbids the pane reading a second source — so an overlay stored beside the snapshot is invisible to every renderer until that signature changes. Change `derive` to take the overlay, and expose per-session lifecycle on the `presentation.rs` / `MonitorSessionRow` surface. This phase draws no markers; it makes `Terminating` reachable and provable end to end, and Phases 13 and 14 add the marker text.
...
- `src/build_monitor/mod.rs` — hold the lifecycle overlay and expose request construction only from fresh actionable sessions.
