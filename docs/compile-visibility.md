...
**Goal:** `Alt-Shift-k` (`Option-Shift-K` on macOS labels) safely terminates exactly the live actionable root rows shown in Output, with final end-to-end proof of the complete feature.
...
Actual problem:
Production `config_path()` uses `dirs::config_dir()`, so XDG variables do not isolate config on macOS; the Phase 13 smoke attempt therefore reached the real user configuration and was stopped without exercising destructive behavior, and Phase 14's destructive smoke was deferred for the same reason.

What exists now:
- The only configuration-root override is test-only, while the live gate needs isolated config, keymap, and theme paths.

What should change:
- Either add one supported runtime configuration-root override covering config, keymap, and themes through a shared resolver used by `src/config.rs`, `src/tui/keymap/load.rs`, and `src/themes/paths.rs`, or require the live matrix to run under a documented disposable macOS user environment.
...
- Ask `BuildMonitor` to create one opaque `OutputBuildSetTerminationAuthorization` from the current exact all-actionable root set. `OutputBuildSetTerminationConfirmationDisplay` freezes the selected-row display identity and exact target summaries separately from that aggregate. Replace `ProjectList::display_path_for_row() -> Option<DisplayPath>` with `ProjectListRowDisplayPathResolution::{Resolved(DisplayPath), RowUnavailable}` in `src/tui/project_list/list.rs`, then construct the confirmation through `OutputBuildSetTerminationConfirmationDisplayResolution::{Ready(OutputBuildSetTerminationConfirmationDisplay), SelectedRowUnavailable}`; do not retain optional display data. UI code never reassembles, combines, or subsets authority from displayed scope/session IDs. Do not add eager invalidation routing: the Phase 12 submission owner already accepts revision churn when `CoveredScopeRoots` remain equal and returns `CoveredScopeRootsChanged` or `SnapshotNotActionable` before mutation when they do not. A project-list or metadata revision bump that leaves the covered roots identical republishes `PendingWithRetained` and remains actionable.
- Add output-build-set termination as a `Ready` `ConfirmAction::TerminateOutputBuildSet` by extending only `execute_confirmed_action`, `finish_clean_metadata_confirmation`, `confirm_action_body`, and `render_confirm_popup`; reuse Phase 13's modal-first keyboard and mouse behavior unchanged.
- A build starting after confirmation is never added to destructive authority. Compare the frozen target identities with the current displayed root rows at submission and carry `AdditionalBuildExclusion::{NoAdditionalBuilds, Excluded { count }}` into completion so App reports how many newer builds were not included without authorizing them. A root that already exited is `gone`, never replaced by a new process at the PID.
- Phase 14 shipped selected-specific `App::submit_selected_build_termination` restoration and a selected-build completion toast that reads the first terminal record. Extract one shared confirmed-request operation for `BuildTerminationTransactionTargetSet::{SelectedBuild, OutputBuildSet}` that owns worker-starting/unavailable restoration, every `BuildTerminationSubmission` result, `BuildTerminationDeadline::from_submission_time`, immediate owned-token fan-out, and one-shot `BuildTerminationCompletionTransition`. Carry the target-set kind plus `AdditionalBuildExclusion` through asynchronous completion so selected and multi-root wording are chosen from semantic state; do not introduce an output-set-specific transaction path or completion ledger.
- Reuse Phase 14's per-root terminating, gone-after-signal, already-gone, survivor, and error projections from `BuildTerminationLifecycleRegistry` and `BuildTerminationTerminalRecord::{session_completion, aggregate_completion, target_results}`. Add only transaction-identity grouping and output-set aggregate presentation, retaining the existing row-independent records until scope change, replacement build, or monitor off without storing another aggregate.
...
- `src/tui/app/confirm_action.rs` — output-build-set confirmation payload retaining `OutputBuildSetTerminationAuthorization` plus `OutputBuildSetTerminationConfirmationDisplay`, and the new exhaustive action arm.
- `src/tui/app/mod.rs` — extract the shared target-set submission/completion operation and create, submit, and reconcile output-build-set requests.
- `src/tui/input/dispatch.rs` — extend `execute_confirmed_action` for `TerminateOutputBuildSet`; keep generic modal input unchanged.
- `src/tui/integration/framework_keymap/output_pane.rs` — availability/dispatch for `alt-shift-k`.
- `src/tui/project_list/list.rs` — replace the optional row display path with `ProjectListRowDisplayPathResolution` for confirmation construction.
- `src/build_monitor/mod.rs` — read-only output-build-set availability and reuse of the existing authorization/submission/completion owner.
- `src/build_monitor/termination/authority.rs` and `src/build_monitor/termination/transaction.rs` — rename the scope-named authorization surface, add `OutputBuildSetTerminationAvailability` and `AdditionalBuildExclusion`, and retain equal-roots submission revalidation without eager invalidation.
- `src/build_monitor/termination/lifecycle.rs` and `src/build_monitor/termination/mod.rs` — carry `BuildTerminationTransactionTargetSet` and `AdditionalBuildExclusion` through existing completion projections.
- `src/tui/panes/output/presentation.rs` — `OutputBuildSetTerminationConfirmationDisplay`, its semantic resolution type, and transaction-grouped aggregate projections from existing registry records; never store a second aggregate or optional display data.
- `src/tui/panes/output/monitor_render.rs` — output-build-set aggregate outcome rendering. Same reason as Phase 12: `render.rs:51-60` only reconciles the cursor and delegates, and every column/header/indicator draw is in `monitor_render.rs`.
- `src/tui/render.rs` — extend `confirm_action_body` and `render_confirm_popup` for output-build-set confirmation, plus target-set-aware completion toasts.
- `src/config.rs`, `src/tui/keymap/load.rs`, and `src/themes/paths.rs` — if the recommended isolation decision is approved, resolve config, keymap, and theme paths from one supported `CARGO_PORT_CONFIG_DIR` root before running the live matrix.

**Constraints from prior phases:** **The output build set is exactly Phase 8's scope-filtered presentation snapshot — the same root rows the Output pane renders.** Do not derive an independent set by re-applying `MonitorScopeKey` or `BuildScopeKey` to `BuildClassification::build_sessions()`: `BuildMonitor` is the single filtering site precisely so the set the user is looking at and the set this phase terminates cannot disagree. One consequence follows mechanically from that equality: an out-of-scope owned run survives Phase 8's filter, so it is inside this output build set. Add an acceptance-gate case proving output-build-set termination signals a live owned run whose checkout root is outside the current `BuildScopeKey` because the Output pane is showing it. Phase 12's scope-named authorization freezes the exact all-actionable set and submission revalidates only actionable state plus equal `CoveredScopeRoots`; rename that semantic surface for the output build set without adding a second invalidation owner. Phase 12 terminal records already carry per-target, session, and aggregate completion independently of rows. A successful owned `Sent` outcome leaves the target `Terminating`; only the correlated `OwnedRunEvent::Finished` proves reap, and a missing `Finished` reaches the five-second deadline/retry-unavailable state. Phase 13 owns `confirmation_modal_state` and generic modal-first input. Phase 14 shipped `BuildTerminationDeadline::from_submission_time`, `BUILD_TERMINATION_TIMEOUT`, immediate owned-token fan-out including synchronous completion, `BuildTerminationCompletionTransition`, detailed terminal records beside live columns, and an actionable dispatch fixture, but its App submission/restoration and completion wording are selected-build-specific; extract the common operation rather than copying those branches. **Phase 10 left the legacy `KillScopedBuilds` action inert, and Phase 14 leaves it that way:** the no-op is now `src/tui/input/dispatch.rs:914` and the disabled shortcut arm is `src/tui/integration/framework_keymap/output_pane.rs:79`. Rename the Rust action to `TerminateOutputBuildSet` while retaining `kill_scoped_builds = "alt-shift-k"` only as the existing TOML compatibility key; `tests/assets/default-keymap.toml:70-71` also pins `kill_selected_build = "alt-k"`. Phase 5 exact scope generations bind the frozen set; UI code never reconstructs, combines, or subsets it. Phase 4 completed-producer pinned output remains outside output-build-set authority, while a live owned monitor row remains inside.

**Acceptance gate:** `bash ~/.claude/scripts/delegate/verify.sh check cargo-port`, `bash ~/.claude/scripts/delegate/verify.sh test cargo-port`, and `bash ~/.claude/scripts/delegate/verify.sh lint cargo-port` are green; automated tests prove read-only output-build-set availability never consumes authority, all-or-refuse actionability, opaque authorization retention without UI reconstruction/combination/subsetting, equal-root revision churn acceptance and incompatible-root refusal at submission, root deduplication, `AdditionalBuildExclusion` reporting, gone-versus-reused identity, modal priority, shared selected/single-root and output-set/multi-root submission and completion wording, immediate owned-token fan-out, and one completion transition per transaction with exact output-set outcomes. Signal acceptance alone draws no gone-after-signal marker, while missing owned `Finished` reaches the five-second deadline/retry-unavailable state. Completed-producer pinned output is excluded while a live out-of-scope owned run shown as a monitor row remains included. Toggle-off behavior matches the resolved pending decision; under the recommendation it stops new compile-monitor work/actions while an already-confirmed transaction continues to completion or deadline without toggle-off signaling any build. The generated keymap fixture preserves the Phase 10 TOML keys; the live verification matrix runs only after the resolved isolation prerequisite is in place, with any platform-observed-only limitation recorded without weakening safety.
