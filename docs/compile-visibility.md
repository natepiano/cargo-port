...
Two facts Phase 7 settled that this decision has to account for:
- The narrowing input is already at the worker. `classify_demand` receives `build_scope_key` and today only logs its root counts (`src/build_monitor/build_classifier.rs:489-493`), and `SessionScope::Resolved { root }` plus `shares_resolved_root` (`src/build_monitor/session.rs:66-90`) gives the comparison directly. Filtering at the worker is now cheaper than the recommendation assumed.
- A session carries only a `CanonicalCheckoutRoot`, never a workspace root, so the filter is checkout-root containment — not the two-sided root comparison used for scope-key equality.

Because of the first fact the plan must pick **one** site — the worker, where the key already is, or `BuildMonitor`, where the recommendation puts it. Naming both would honor the scope key in two places, which is the exact divergence this decision exists to prevent.

Recommendation:
Filter here, in `BuildMonitor`, and state that the worker does not filter. One filter means the set the user sees and the set a kill acts on cannot diverge, which is the property Phases 11–13 depend on; keeping the worker host-wide also keeps the out-of-scope owned run classifiable, which is why classification was made host-wide in the first place. Applying it means adding a Spec bullet and an Acceptance-gate clause to this phase proving an out-of-scope live Cargo session is absent from the snapshot while the out-of-scope owned pin survives, and adding a Constraints line to Phases 9, 12, and 13 saying the snapshot is already scope-filtered.
...
**Acceptance gate:** `bash ~/.claude/scripts/delegate/verify.sh check cargo-port`, `bash ~/.claude/scripts/delegate/verify.sh test cargo-port`, and `bash ~/.claude/scripts/delegate/verify.sh lint cargo-port` are green; render/state tests prove distinct non-actionable pending-index, empty-non-Rust, ambiguous-ownership, and unresolved-path states; semantic absent versus retained-output producer states; run N retained output remains attributed to N while N+1 is queued/starting; empty enabled focus; full-width single session; readable/windowed multiple columns; selected-column visibility; stable row/session fallback; same-PID exec invalidates prior cursor and creates new session/activity identities; unattributed section non-actionability; owned output joined once and pinned across scope changes; and monitor-off output matches prior behavior. This phase also removes the `#[cfg(test)]` gate — not a `dead_code` suppression — from `MonitorScopeResolutionRevision::monitor_workspace_index_readiness` (`src/tui/compile_visibility/scope.rs:146`, returning `MonitorWorkspaceIndexReadiness` at `:114`), whose empty-state rendering is specified above, and from the `BuildSession` presentation accessors this phase is the first production reader of — `operative_cargo_command`, `cargo_subcommand_recognition`, `session_scope`, `root_observation`, `session_target_directory`, and `build_profile` (`src/build_monitor/session.rs:444-479`), plus `SessionRootObservation::root_pid` and `first_observed_at` (`:398-403`) for the root-PID and elapsed-time columns. The two `root_identity` accessors (`:395`, `:461`) stay gated — their first production caller is Phase 11's pre-signal revalidation.

### Phase 10 — Monitor navigation, toggle, and owned-output coexistence · status: todo

**Pending decision:** This phase adds the `Shift-C` action that makes
`App::toggle_compile_visibility` reachable, which puts
`refresh_compile_monitor_scope_if_on` on the input-event path — every key and
mouse event calls `sync_selected_project` (`src/tui/input/dispatch.rs:72`), and
that reaches `App::workspace_index_readiness`, a `&mut self` method that runs
`rebuild_if_changed` while holding the metadata-store mutex.
...
`refresh_compile_monitor_scope_if_on` returns before `workspace_index_readiness()`
when the state is `Off`, so nothing is paid until the user enables monitoring;
and what remains while enabled is one mutex acquisition per input event
(including mouse-motion), not a filesystem walk.

The obvious remedy regresses behavior: a non-rebuilding readiness accessor
cannot know whether the index is fresh, so it must report either `Current`
(wrong when stale) or `RetainedLastAccepted` (wrong when fresh). Either makes
`MonitorScopeResolutionRevision` differ between the metadata-handler path and
the keystroke path, and that field participates in `requires_scope_replacement`
equality for non-actionable resolutions — so every keystroke would advance the
monitor generation with nothing changed. Options: (a) accept the per-event
mutex, (b) add a third readiness state meaning "not revalidated on this call"
that is excluded from replacement equality, (c) track index currency on `App`
without taking the lock.

**Decide before Phase 8, not before the keybinding.** Phase 7 confirmed the path in source: `refresh_compile_monitor_scope_if_on` → `resolve_compile_monitor_scope` (`src/tui/app/mod.rs:1318-1322`) → `workspace_index_readiness()`, which takes the metadata mutex and may replace the `Arc` on every call. Option (b) changes `MonitorScopeResolutionRevision` equality — and that equality is what drives generation advance, which is what Phase 8's `PendingWithRetained` display rule is built on. Choosing (b) after Phase 8 ships would mean Phase 8 was built against resolution semantics that Phase 10 then changed.
...
- Build the set from the current monitor scope only. This runs `tui`-side, so the full `MonitorScopeKey` is available and is the authority here; the monitor snapshot it filters is keyed by `BuildScopeKey`, so join through `ActiveMonitorState::build_scope_actionability()` and match on `BuildScopeActionability::Actionable`, rather than comparing the two keys directly or calling `BuildScopeKey::from(&monitor_scope_key)`. Phase 7 established `build_scope_actionability` (`src/tui/compile_visibility/mod.rs:155-167`) as the one entry point through which a monitor scope reaches build classification, precisely so the five resolution states are not restated downstream; a direct `From` call bypasses it and would let this phase build a destructive set from a scope that never passed the actionability check. Exclude pinned owned output outside the selected scope, completed runs, tombstones, unattributed compiler units, and duplicate/nested references to the same root.
