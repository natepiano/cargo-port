...
Architect review of Phases 11–13 against the code Phase 10 actually shipped. Seventeen findings; all applied to the Work Orders above.

**Cross-cutting answer to the question that triggered the review** — Phase 10's largest failure was that its Work Order assumed a motion API on `OutputCursor` that did not exist. No remaining Work Order repeats it: every type and method Phases 11–13 name as already present does exist, including `ProcessObserver` (`src/process_observation/mod.rs:1538`), `OwnedProcessGroupTerminationCapability` (`src/tui/state/inflight.rs`), and both `#[cfg(test)]` root-identity accessors (`src/build_monitor/session.rs:404`, `:465`). Four line references had drifted and are corrected in place.

**Applied without a gate** — Phase 12's cursor-exposure directive is replaced: the pane computes and returns a named answer type rather than exposing any cursor enum, matching the shipped `owned_column_selection` pattern. Its target resolver is now stated as a separate derivation from the motion resolver, its availability rule is restated as column identity rather than row kind, and its terminating/tombstone markers move from `render.rs` to `monitor_render.rs` — Phase 13's identical error corrected the same way. Phase 13 gains `async_tasks/poll.rs` (where `CoveredScopeRoots` invalidation has to run) and loses the keymap fixture from its work list (Phase 10 already pinned it). Both phases now name the two lines that keep Alt-K inert. Phase 12's modal-precedence tests are redirected to the `dispatch.rs` harness, the only one that can reach the modal layer. Phase 11 now requires whichever phase owns the lifecycle overlay to also name the `OutputPresentation::derive` signature change — without it nothing can draw `Terminating`.

**Deferred as a pending decision** — Phase 12 gained one: its modal precedence and `y`/`n`/Esc rules are not new wiring but changes to the confirm modal shared by five existing actions, and shipped dispatch currently runs the Output cancel preflight *before* the confirm handler. Whether to scope those changes to the new action or change the shared modal — and whether that becomes its own phase ahead of Phase 12 — is unresolved.
...
**Pending decision: Own and serialize Cargo Port-owned termination authority**

Actual problem:
Phase 4's `OwnedRun` solely owns a non-cloneable `OwnedProcessGroupTerminationCapability`, while this Work Order asks monitor snapshots and retained modal authorizations to own that capability and also combines platform capability work with the complete bounded transaction.

What exists now:
- `OwnedRun` retains the only verified group capability through running/stopping states, and its completion worker owns the child wait.
- Phase 11 currently combines owned child/group serialization, external platform adapters, the terminator worker, BuildMonitor authorization, descendant admission, and bounded reconciliation.

What should change:
- Introduce an `OwnedRunProcessActor` that alone owns the child wait and group capability and exposes opaque run-bound termination authorization through a serialized endpoint; monitor/UI state never clones or decomposes the capability.
- Split this Work Order into a termination-foundation phase for the owned actor, external platform adapters, and worker boundary, followed by a BuildMonitor authorization/transaction phase; renumber the two UI phases afterward. **The renumbering has a concrete cost:** Phases 12 and 13 refer to "Phase 11's `SelectedBuildTerminationAuthorization`" / "Phase 11's one bounded transaction" by number in five places across their Spec and Constraints, all of which must be rewritten if this split is taken.

Recommendation:
Adopt the actor endpoint and split before implementing destructive monitor actions, preserving one authority owner and making each safety boundary independently reviewable.

**Pending decision: Where per-session lifecycle state lives across a 500 ms snapshot replacement**

Actual problem:
No phase owns lifecycle state that survives a poll cycle. Phase 8's `record_classification` replaces `monitor_snapshot` wholesale every cycle, and `BuildMonitor` stores only `monitor_snapshot` plus `live_session_ids` (`src/build_monitor/mod.rs:82-85`). A `Terminating` transition (this phase), a gone-after-signal tombstone for the selected build (Phase 12), and a scope-wide tombstone (Phase 13) would each be erased by the next cycle and again by `replace_scope`.

What exists now:
- `record_classification` (`src/build_monitor/poll.rs:42-56`) builds a fresh `MonitorData` per cycle and drops the prior one; nothing carries forward except the retention path `superseded_by_scope` uses.
- `BuildMonitor::live_session_ids` is a `BTreeSet<BuildSessionId>` derived from the snapshot's rows and cleared alongside them (`poll.rs:45-48, 62-64, 81-86`), exposed only through a `#[cfg(test)]` accessor — a second source of truth about which sessions are live, with no production reader and no phase currently claiming it.
- Phase 13 introduces `src/build_monitor/aggregate.rs`, which arrives one phase *after* Phase 12 first needs the storage.

What should change:
- This phase owns a `BuildMonitor`-held lifecycle overlay keyed by `BuildSessionId`, outside `monitor_snapshot`, surviving both `record_classification` and `replace_scope`, with explicit eviction rules rather than implicit clearing.
- Adopt `live_session_ids` as that overlay's key set so there is one source of truth, or delete it here if the overlay supersedes it. Its current references are `src/build_monitor/poll.rs:50-56, :68, :90`.
- Add `src/build_monitor/snapshot.rs` and `src/build_monitor/poll.rs` to this phase's **Files** — the overlay has to be read where the snapshot is rebuilt.
- **Name the path that carries the overlay to the renderer, or nothing can draw `Terminating`.** `OutputPresentation::derive` takes only `&MonitorSnapshot` plus owned-run state (`src/tui/app/mod.rs:1207-1213, 1221-1229`), and Phase 9's single-presentation constraint forbids the pane reading a second source — so an overlay stored beside `monitor_snapshot` is invisible to every renderer. Whichever phase owns the overlay must also name the `derive` signature change and the `presentation.rs` / `MonitorSessionRow` surface that exposes per-session lifecycle. This binds Phase 12's terminating/tombstone markers and Phase 13's scope-wide tombstones equally.

Recommendation:
Own the overlay here, in the same phase that introduces the first lifecycle transition (`Terminating`), and fold `live_session_ids` into it rather than leaving two answers to "which sessions are live". Phases 12 and 13 then store tombstones in existing storage instead of each inventing its own.
...
- **`n`/Esc is not how the shared handler works.** `handle_confirm_key` (`dispatch.rs:615-663`) calls `take_confirm()` unconditionally, so *any* key that is not `y` cancels. "All other keys do nothing" is a semantics change to that shared modal.
- This phase has also grown a third shared-subsystem change it does not name: extending `OutputPresentation` to carry lifecycle state (see Phase 11's second pending decision).
...
- If the shared change is chosen, consider splitting a "modal-precedence and confirm semantics" phase ahead of this one so each safety boundary is reviewable on its own — the same split Phase 11's first pending decision already proposes for termination authority.
...
**Pending decision: One derivation of "may this be acted on", not two**

Actual problem:
Two values now partition the seven `MonitorSnapshot` variants identically. `MonitorDisplay::Rows { monitor_staleness }` is what the renderer draws off (`src/build_monitor/snapshot.rs:320-342`, production since Phase 9); `MonitorSnapshot::actionability()` is what this phase and Phase 13 authorize destruction off (`:304-318`, still `#[cfg(test)]`). `MonitorStaleness::Live` and `MonitorDataActionability::Actionable` cover exactly the same variants today. Unless one is derived from the other, the staleness marker the user sees and the availability of the kill action are two independent answers to the same question, and a future variant can make them disagree — which is precisely what Phase 13's Constraints exist to prevent.

What exists now:
- `monitor_display()` — production, drives the pane's rows and stale marker.
- `actionability()` — test-gated, no production reader; Phase 9's Work Order claimed the un-gate but correctly shipped `monitor_display()` instead, having no reader for the other.

What should change:
- Derive one from the other so a variant can only ever have one answer. Either `actionability()` becomes a function of `monitor_display()`, or both become projections of one private classification of the seven variants.

Recommendation:
Derive `actionability()` from `monitor_display()`. The renderer's classification is the one with a production reader and the one the user can see, so a disagreement would show up as a visible stale marker beside an available kill action. Phase 10 touched neither function, so this recommendation is unchanged — and it strengthened the argument: `indicator_line(monitor_staleness)` now draws in every monitor state (`src/tui/panes/output/monitor_render.rs:462-463, 493-494`), so the staleness marker is unconditionally on screen right where the kill affordance would appear. Make the change in this phase, since this phase is the first production reader of `actionability()`; add an acceptance-gate case asserting the two agree across all seven variants.
...
- `src/build_monitor/snapshot.rs` — un-gate `MonitorSnapshot::actionability()` (`:304-318`), `MonitorDataActionability` (`:253`), and `MonitorObservation` (`:263`). Phase 9's Work Order claimed this un-gate but shipped `monitor_display()` as its production reader instead; this phase is the first reader of the other three. Keeping authorization construction inside `BuildMonitor`, as Phase 11 requires, means no `MonitorData` re-export from `src/build_monitor/mod.rs` is needed.

**Constraints from prior phases:** The selectable set is Phase 8's scope-filtered presentation snapshot, which `BuildMonitor` narrowed once; never re-filter it and never widen back to `BuildClassification::build_sessions()`. Use Phase 10's framework action and platform label; retain and submit only Phase 11's `SelectedBuildTerminationAuthorization` without reconstructing or substituting a scope-wide aggregate. **Phase 10 left both kill actions deliberately inert in exactly two places, and this phase must split each:** `dispatch_output_action` no-ops both kill arms at `src/tui/input/dispatch.rs:904-906`, and `Shortcuts::state` returns `Disabled` for both in a single match arm at `src/tui/integration/framework_keymap/output_pane.rs:68-75`. Wire `KillSelectedBuild` here and leave `KillScopedBuilds` disabled for Phase 13. Phase 5 scope changes make an open request invalid, Phase 9 owns exec-sensitive selection identity/fallback, and the legacy Running Targets termination capability remains unrelated.
...
- `src/build_monitor/aggregate.rs` — per-root and aggregate results/tombstones.
- `src/tui/app/async_tasks/poll.rs` — invalidate the retained authorization on `CoveredScopeRoots` inequality. This is the path a new snapshot lands on, so it is where the comparison has to run; `CoveredScopeRoots` itself lives at `src/build_monitor/scope.rs`.
...
**Constraints from prior phases:** **The scope-wide kill set is exactly Phase 8's scope-filtered presentation snapshot — the same value the Output pane renders.** Do not derive an independent set by re-applying `MonitorScopeKey` or `BuildScopeKey` to `BuildClassification::build_sessions()`: `BuildMonitor` is the single filtering site precisely so the set the user is looking at and the set this phase terminates cannot disagree, and a second derivation reintroduces that disagreement in the one place where it destroys work. One consequence follows mechanically from that equality and must be asserted rather than left implicit: an out-of-scope owned run survives Phase 8's filter, so it is inside this kill set. Add an acceptance-gate case proving scope-wide termination signals a live owned run whose checkout root is outside the current `BuildScopeKey` — it is in the set because the Output pane is showing it, and "stop everything shown" that silently spares one column would be the worse surprise. Phase 12 establishes modal selected termination; reuse its input path while retaining Phase 11's distinct `ScopeTerminationAuthorization` for set-wide authority — including whatever Phase 12's modal-precedence pending decision settles, since this phase inherits that modal rather than defining its own. **Phase 10 left `KillScopedBuilds` deliberately inert in two places and Phase 12 leaves it that way; this phase is what enables it:** the no-op arm in `dispatch_output_action` (`src/tui/input/dispatch.rs:904-906`) and the `Disabled` arm in `Shortcuts::state` (`src/tui/integration/framework_keymap/output_pane.rs:68-75`). The keymap fixture needs no edit — `tests/assets/default-keymap.toml:70-71` already pins `kill_scoped_builds = "alt-shift-k"` and `kill_selected_build = "alt-k"` from Phase 10; the acceptance gate re-asserts it rather than producing it. Phase 11's aggregate/transaction semantics plus Phase 5 exact scope generations bind the frozen set; UI code never reconstructs, combines, or subsets it. Phase 4 pinned owned output remains outside unrelated scope-wide authority.
