...
**Implications for remaining phases:**
- Phase 10 makes `toggle_compile_visibility` reachable, which puts scope refresh on the input-event path. See the `**Pending decision:**` block on Phase 10 — resolve before wiring the keybinding.
...
**Pending decision: Where an external column's off-screen activity rows live**

Actual problem:
This Work Order's Up/Down/Home/End/Page traversal covers "the selected column's complete body", but no per-column vertical scroll state exists. `render_column` lays activity rows out from the top of the column area and `row_rect` returns `HitRegion::Truncated` for anything past the bottom (`src/tui/panes/output/hit_map.rs:108`, `monitor_render.rs:498-545`), and the pane's single `Viewport` is sized to the owned captured output only (`render.rs:49-58`). A cursor moved past the drawn area therefore lands on a row that is neither drawn nor hittable. The scope-level unattributed section has the same limit and already counts what it drops (`UnattributedSectionLayout.hidden`, `monitor_render.rs:445-483`).

What exists now:
- One `Viewport` on the Output pane, sized from `owned_captured_output_height` for the owned body only.
- External column rows drawn top-down with no offset; anything past the column height is dropped by the hit map, not scrolled to.
- `UnattributedSectionLayout { height, hidden }` already carries a hidden count, rendered as `— N more`.

What should change:
- Either add per-column vertical scroll state (a scroll offset per session column, reconciled against the cursor the way the owned viewport already is), so traversal reaches every row; or restrict traversal to drawn rows and make the hidden remainder visible the way the unattributed section already does.

Recommendation:
Add the per-column scroll offset. Restricting traversal makes a column's contents depend on pane height in a way the user cannot act on, and the acceptance gate's "traverse the complete ordered session list" already commits to reaching off-screen columns horizontally — reaching off-screen rows vertically is the same promise. Place it in this Work Order's Spec and Files (`src/tui/panes/output/pane.rs`, `monitor_render.rs`), and extend the acceptance gate with a case that traverses a column taller than the pane.
...
- Split this Work Order into a termination-foundation phase for the owned actor, external platform adapters, and worker boundary, followed by a BuildMonitor authorization/transaction phase; renumber the two UI phases afterward.
...
- Adopt `live_session_ids` as that overlay's key set so there is one source of truth, or delete it here if the overlay supersedes it.
...
- Expose the action only when the selected column's session comes from a snapshot whose `MonitorSnapshot::actionability()` is `MonitorDataActionability::Actionable` **and** that session carries authority-bearing `ActionableBuild::Owned` or `ActionableBuild::External`. Phase 8 shipped `actionability()` as `Actionable` for **both** `Fresh` and `PendingWithRetained` (`src/build_monitor/snapshot.rs:302-317`), so a retained snapshot republished after a scope replacement keeps termination available — do not restate the rule as "fresh session only". `Off`, `Pending` (without retention), `Stale`, `StaleWithRetained` (added by Phase 9, `snapshot.rs:242`), and `Unavailable` are `NotActionable` and expose nothing. Headers and activity rows may invoke it; unattributed, observed-only, completed, killed, failed-unrefreshed, and terminating sessions cannot.
- **Resolve the target session by identity, not by cursor index.** `reconcile_cursor` runs from the render body (`src/tui/panes/output/render.rs:66`), so between a poll result landing and the next frame the cursor's column index and retained `BuildSessionId` still describe the previous snapshot. Look the session up by its exec-sensitive `BuildSessionId` against the current snapshot at action time; a mismatch is a refusal, never a different process.
...
- `src/tui/panes/output/pane.rs` — selected session/row lookup. `OutputCursor` already holds `OutputCursorColumn::{Detached, Session(BuildSessionId)}` privately (`src/tui/panes/output/selection.rs:199-205`) — exactly this value, with the absence case already named. Widen that type's visibility and expose it; do not add an accessor returning `Option<&BuildSessionId>`.
