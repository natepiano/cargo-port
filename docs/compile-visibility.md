...
without taking the lock. Decide before wiring the keybinding.
...
- Build the set from the current `MonitorScopeKey` only. This runs `tui`-side, so the full key is available and is the authority here; the monitor snapshot it filters is keyed by `BuildScopeKey`, so join through `BuildScopeKey::from(&monitor_scope_key)` rather than comparing the two directly. Exclude pinned owned output outside the selected scope, completed runs, tombstones, unattributed compiler units, and duplicate/nested references to the same root.
