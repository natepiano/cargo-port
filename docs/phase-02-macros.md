...
| Change | Reason |
|---|---|
| Introduce `pub trait ActionEnum` in `tui_pane::keymap::action_enum` | Doc designates `Action: ActionEnum + 'static` as the bound on every `Shortcuts` / `Navigation` / `Globals` impl. The bound has to name a real trait, not a bag of supertraits restated in three places. |
...
pub use crate::keymap::action_enum::ActionEnum;
...
| `ActionEnum` trait | does not exist | new, in `tui_pane::keymap::action_enum` |
