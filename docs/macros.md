...
- `impl Action for $Name` — there is no `Action` trait in
...
| Change | Reason |
|---|---|
| Introduce `pub trait Action`, defined in `keymap/action_enum.rs` and re-exported flat as `tui_pane::Action` | Doc designates `Action: Action + 'static` as the bound on every `Shortcuts` / `Navigation` / `Globals` impl. The bound has to name a real trait, not a bag of supertraits restated in three places. |
| `impl $crate::Action for $Name` is generated by the macro | Single source of truth — every action enum ships its `Action` impl through this macro and through nothing else. |
...
| `from_toml_key` becomes a trait method on `Action` | TOML parsing in `tui_pane` is generic over the action type; it needs a trait-dispatched constructor. |
...
| `bar_label()` added to the `Action` trait (Phase 5) | The bar reads `action.bar_label()` once per slot per frame; making it a trait method gives generic bar code a single source for the static label. |
...
### 2.3 `Action` trait — formal definition
...
2. The `impl $Name { ... }` inherent block is gone; every method moves
   into `impl $crate::Action for $Name`. Callers that previously
...
5. `$crate::Action` paths so the macro works when invoked from
...
| `Action` trait | does not exist | new, defined in `keymap/action_enum.rs`, re-exported flat as `tui_pane::Action` |
...
satisfy the `Action` super-trait bounds.
