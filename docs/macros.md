...
| Change | Reason |
|---|---|
| Introduce `pub trait ActionEnum`, defined in `keymap/action_enum.rs` and re-exported flat as `tui_pane::ActionEnum` | Doc designates `Action: ActionEnum + 'static` as the bound on every `Shortcuts` / `Navigation` / `Globals` impl. The bound has to name a real trait, not a bag of supertraits restated in three places. |
...
pub use crate::keymap::{ActionEnum, Bindings, KeyBind, ScopeMap};
...
| `ActionEnum` trait | does not exist | new, defined in `keymap/action_enum.rs`, re-exported flat as `tui_pane::ActionEnum` |
