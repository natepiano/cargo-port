...
- **14.7 — `AppGlobalAction` expansion + retire local `action_enum!` macro.** Three scoped jobs:
  ...
  3. Delete only the local `macro_rules! action_enum` block at `src/keymap.rs:215-250` after every binary action enum has migrated to `tui_pane::action_enum!`.
...
- Finder `Pane::mode()(ctx)` returns `Mode::TextInput(finder_keys)` while open, `Mode::Navigable` otherwise. (Pending 14.6.)
- Finder migration: typing `'k'` in the search box inserts `'k'` into the query (handler is sole authority — vim keybinds in other scopes do not fire). (Pending 14.6.)
...
- ✅ Eight per-pane bar snapshots landed in 14.3 / 14.4 (Git / Targets / Lints / Lang / Cpu / CiRuns / ProjectList plus the in-Nav-region ProjectList paired-slot check). Two remaining for 14.5 / 14.6: Output and Finder. The pre-Phase-14.4 deferral list ("nine remaining" snapshots) is therefore obsolete — only Output and Finder are still pending.
...
- The inherent-method facade blocks on `PackageAction` / `GitAction` / `TargetsAction` / `CiRunsAction` / `LintsAction` / `ProjectListAction` / `OutputAction` / `FinderAction` (each pane's `pub const ALL` + `pub fn toml_key` / `description` / `from_toml_key` forwarders to the `tui_pane::Action` trait impl, added during 14.2–14.6 to keep legacy call sites compiling). After Phase 18 retires the legacy `src/keymap.rs` `ResolvedKeymap` plumbing the facades have no consumers — call sites read `<X as tui_pane::Action>::method` through the trait directly. **Excluded:** `LangAction` and `CpuAction` (defined locally in `src/tui/framework_keymap.rs` during 14.4a) have no facade and no `ResolvedKeymap` consumers — Phase 18 has no work for those two enums; they survive into the post-Phase-18 binary as-is.
