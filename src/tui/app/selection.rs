...
impl App {
    ...
    pub(super) fn all_group_members(node: &ProjectEntry) -> impl Iterator<Item = &Project> {
    ...
    pub(super) fn all_vendored_projects(node: &ProjectEntry) -> impl Iterator<Item = &Project> {
    ...
    pub(super) fn fit_name_for_node(node: &ProjectEntry, live_worktrees: usize) -> usize {
    ...
    /// Returns the `ProjectEntry` when a root row is selected (not a member or worktree).
    pub fn selected_node(&self) -> Option<&ProjectEntry> {
        match self.selected_row()? {
