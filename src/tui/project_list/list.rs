...
use super::visible_rows::VisibleRow;
use crate::ci;
use crate::ci::CiRun;
...
impl ProjectList {
    ...
    pub fn resolved_root_labels(&self, include_non_rust: bool) -> Vec<String> {
        ...
        {
            let suffixes = grouping::shortest_unique_suffixes(
                &indices
                    .iter()
    ...
    fn rust_info_at_path_mut(&mut self, target: &Path) -> Option<&mut RustInfo> {
        self.roots
            .values_mut()
    ...
    fn vendored_at_path_mut(&mut self, target: &Path) -> Option<&mut VendoredPackage> {
        self.roots
            .values_mut()
    ...
    fn checkout_root_for(&self, target: &Path) -> Option<AbsolutePath> {
        self.roots
            .values()
    ...
    fn git_repo_for_mut(&mut self, path: &Path) -> Option<&mut GitRepo> {
    ...
    /// at `path`.
    fn primary_ahead_behind_for(&self, path: &Path) -> Option<(usize, usize)> {
        let checkout = self.git_info_for(path)?;
        let repo = self.repo_info_for(path)?;
    ...
    fn unpublished_ci_branch_name(&self, path: &Path) -> Option<String> {
    ...
    fn promote_to_worktree_group(&mut self, path: &Path, group: WorktreeGroup) -> bool {
        ...
        for entry in self.roots.values_mut() {
            ...
            let inserted = match &mut entry.root_item {
                RootItem::Rust(RustProject::Workspace(ws)) => {
                    grouping::try_insert_member(ws, &item_path, &item)
                },
    ...
    pub fn regroup_members(&mut self, inline_dirs: &[String]) {
        for entry in self.roots.values_mut() {
            match &mut entry.root_item {
                RootItem::Rust(RustProject::Workspace(ws)) => {
                    grouping::regroup_workspace(ws, inline_dirs);
                },
                RootItem::Worktrees(group) => {
    ...
    pub fn regroup_top_level_worktrees(&mut self) {
        ...
        while index < self.roots.len() {
            let Some(identity) =
                grouping::linked_worktree_identity(&self.roots[index].root_item).cloned()
            else {
                index += 1;
                continue;
            };
            let Some(mut target_index) =
                grouping::find_matching_worktree_container(&self.roots, index, &identity)
            else {
                index += 1;
                continue;
            };
            ...
            let attached = grouping::try_attach_worktree(
...
impl ProjectList {
    ...
    const fn mutate(&mut self, include_non_rust: bool) -> SelectionMutation<'_> {
        SelectionMutation {
            project_list: self,
...
impl ProjectList {
    ...
    fn path_for_row(&self, row: VisibleRow) -> Option<&Path> {
        match row {
    ...
    fn row_matches_project_path(&self, row: VisibleRow, target_path: &Path) -> bool {
        self.path_for_row(row)
...
impl ProjectList {
    ...
    fn expand_path_in_tree(&mut self, target_path: &Path) {
        let (roots, expanded) = self.iter_with_expanded_mut();
        for (ni, entry) in roots.enumerate() {
            match &entry.root_item {
                RootItem::Rust(RustProject::Workspace(ws)) => {
    ...
    fn select_matching_visible_row(&mut self, target_path: &Path, include_non_rust: bool) {
        self.recompute_visibility(include_non_rust);
    ...
    fn collapse_to(&mut self, key: &ExpandKey, target: VisibleRow, include_non_rust: bool) {
    ...
    fn collapse_row(&mut self, row: VisibleRow, include_non_rust: bool) {
        match row {
