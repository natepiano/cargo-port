...
pub(crate) fn process_group_exists(process_group_id: u32) -> bool {
    process_group_pid(process_group_id)
...
#[cfg(unix)]
fn process_group_pid(process_group_id: u32) -> io::Result<Pid> {
    let raw_pid = i32::try_from(process_group_id)
        .map_err(|_| io::Error::new(ErrorKind::InvalidInput, "process group exceeds pid_t"))?;
    Pid::from_raw(raw_pid)
        .ok_or_else(|| io::Error::new(ErrorKind::InvalidInput, "process group must be nonzero"))
}

#[cfg(unix)]
fn signal_process_group(process_group_id: u32, signal: Signal) -> io::Result<()> {
    let target_pid = process_group_pid(process_group_id)?;
