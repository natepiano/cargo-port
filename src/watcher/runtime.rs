...
use super::events;
use super::events::WatcherBackgroundSinks;
use super::probe;
use super::refresh;
use super::roots;
use super::roots::RegisteredRoots;
use crate::channel;
...
use crate::constants::MAX_WAIT;
use crate::constants::POLL_INTERVAL;
use crate::constants::SCAN_METADATA_CONCURRENCY;
use crate::constants::WATCHER_DISK_CONCURRENCY;
use crate::constants::WATCHER_GIT_CONCURRENCY;
...
impl ProjectEntry {
    ...
    pub fn is_alive(&self) -> bool { self.abs_path.join(CARGO_TOML).is_file() }
}
...
impl WatchState {
    pub fn pending(now: Instant, immediate: bool) -> Self {
        Self::Pending {
...
pub struct WatcherLoopState {
    pub projects:        HashMap<AbsolutePath, ProjectEntry>,
    pub project_parents: HashSet<AbsolutePath>,
    pub pending_disk:    HashMap<String, WatchState>,
    pub pending_git:     HashMap<AbsolutePath, WatchState>,
    pub pending_new:     HashMap<AbsolutePath, Instant>,
    pub discovered:      HashSet<AbsolutePath>,
    pub registration:    WatcherRegistrationPhase,
    pub buffered_events: Vec<Event>,
}

#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum WatcherRegistrationPhase {
    #[default]
    Initializing,
    Ready,
}
...
impl WatcherRegistrationPhase {
    pub const fn is_initializing(self) -> bool { matches!(self, Self::Initializing) }
}
...
fn watcher_loop<W: Watcher + Send + 'static>(
    ...
    loop {
        ...
        let notify_events = events::drain_notify_events(notify_rx);
        events::process_notify_events(
            tick,
            &watch_drain,
            notify_events,
        ...
        events::drain_completed_refreshes(
            &disk_done_rx,
            &git_done_rx,
...
pub struct WatchDrainResult {
    pub channel_state:         WatchChannelState,
    pub registration_progress: WatchRegistrationProgress,
}

#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum WatchChannelState {
    #[default]
    Connected,
    Disconnected,
}
...
impl WatchChannelState {
    pub const fn is_disconnected(self) -> bool { matches!(self, Self::Disconnected) }
}

#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum WatchRegistrationProgress {
    Completed,
    #[default]
    Pending,
}
...
mod tests {
    ...
    fn register_watch_roots_reports_per_root_failures() {
        ...
        let (registered, failures) = roots::register_watch_roots(&mut watcher, &dirs);
    ...
    fn startup_registration_introduces_no_overlapping_watches() {
        ...
        let (registered_roots, failures) = roots::register_watch_roots(&mut guard, &watch_roots);
        assert!(
            failures.is_empty(),
        ...
        roots::register_cargo_home_watch(&mut guard, &registered_roots);
        // SAFETY: same serial-test reasoning as the `set_var` above.
        unsafe { std::env::remove_var("CARGO_HOME") };
